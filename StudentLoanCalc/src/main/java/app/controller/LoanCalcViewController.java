...
public class LoanCalcViewController implements Initializable  {
	...
	private void btnCalcLoan(ActionEvent event) {
		...
		for(int i = 0; i < L.totalPayments(); i++) {
			Payment p = L.loanPayments.get(i);
			System.out.println(p.getPaymentNbr() + "  |  " + p.getDueDate() + "  |  "+  p.getPaymentAmt() + "  |  " + p.getAddPayment() + "  |  " + p.getInterestAmt() + "  |  " + p.getPrincipleAmt() + "  |  " + p.getBalance());
		}
	} 
}
